...
class Assistant(Agent):
    def __init__(self) -> None:
        super().__init__(
            instructions="""
You are a friendly coffee shop barista for a brand called Mocha Masti.
...
""",
    ...
    async def save_order(self, context: RunContext) -> str:
        ...
        # Ensure orders directory exists (use absolute path from backend root)
        ...
        filename = os.path.join(orders_dir, f"order-{timestamp}-{safe_name}.json")
